...
export function TransactionDetailsDrawer() {
	const { activeTransaction, setActiveTransaction } = useBorrow();
	const { user } = useAuth();

	if (!activeTransaction) return null;

	const isUserBorrower = user && user.id === activeTransaction.borrower.id;
	const image = isUserBorrower ? activeTransaction.lender.image : activeTransaction.borrower.image;
	...
	const email = isUserBorrower ? activeTransaction.lender.email : activeTransaction.borrower.email;
		...
		<Drawer open={!!activeTransaction} onOpenChange={open => !open && setActiveTransaction(null)}>
			<DrawerContent className="max-h-[90vh]">
				...
				<DrawerFooter className="border-t pt-4 pb-8">
					<BorrowMobileActions data={activeTransaction} showFullButtons />
				</DrawerFooter>
			</DrawerContent>
		</Drawer>
	);
