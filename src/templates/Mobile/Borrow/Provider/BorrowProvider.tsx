...
interface BorrowContextType {
	...
	isBorrowRepayModalOpen: boolean;
	setIsBorrowRepayModalOpen: React.Dispatch<React.SetStateAction<boolean>>;
}
...
export default function BorrowProvider({ children }: GlobalLayoutProps) {
	...
	// Additional States & Functions
	const [isBorrowRepayModalOpen, setIsBorrowRepayModalOpen] = useState(false);
	...
	useEffect(() => {
		if (transactionBorrowResponse?.data) {
			if (apiSearchParams.page === 1) {
				setAccumulatedData(transactionBorrowResponse.data);
			} else {
				setAccumulatedData(prev => {
					// Create a Map to ensure uniqueness by ID
					const newData = transactionBorrowResponse.data || [];
			...
			const totalPages = transactionBorrowResponse.pagination?.totalPages || 1;
			setHasMore(apiSearchParams.page < totalPages);
		}
	}, [transactionBorrowResponse, apiSearchParams.page]);
