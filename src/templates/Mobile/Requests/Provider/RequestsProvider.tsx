...
export default function RequestsProvider({ children }: GlobalLayoutProps) {
	...
	useEffect(() => {
		if (transactionRequestsResponse?.data) {
			if (apiSearchParams.page === 1) {
				setAccumulatedData(transactionRequestsResponse.data);
			} else {
				setAccumulatedData(prev => {
					// Create a Map to ensure uniqueness by ID
					const newData = transactionRequestsResponse.data || [];
			...
			const totalPages = transactionRequestsResponse.pagination?.totalPages || 1;
			setHasMore(apiSearchParams.page < totalPages);
		}
	}, [transactionRequestsResponse, apiSearchParams.page]);
